...
static volatile size_t raw_limit_;		/* for debugging */

#define MARGIN (32 * 1024)
#define DEFAULT_LIMIT ((1024 * 1024) - MARGIN)
...
*/
CTAGS_INLINE size_t getDefaultLimit (void)
{
	size_t cur_size = 0;
	size_t limit;
...
extern size_t stackGuardGetLimit (void)
{
	...
	limit_ = getDefaultLimit ();
	return limit_;
}

extern void stackGuardSetLimit (const size_t limit)
{
	if (dynamicStackGuard)
	{
		size_t defaultLimit = getDefaultLimit ();
		if (limit > defaultLimit)
		{
			error (WARNING, "don't specify stack limit larger than default: %zu > %zu",
				   limit, defaultLimit);
			limit_ = defaultLimit;
			return;
		}
...
extern bool stackGuardCheck (void *dataForDiscardFunc)
{
	...
	size_t limit = stackGuardGetLimit ();
	if (used > limit)
	{
		safe = false;
...
extern char *makeStackGuardHelpDescription (const char *template)
{
	vString *vstr = vStringNewInit (template);
	char buf[64] = {'\0'};

	snprintf (buf, sizeof (buf), "[%zu].", stackGuardGetLimit ());
