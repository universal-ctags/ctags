...
typedef struct
{
  ...
  stringList* operatorList; /* this will hold a list of operator */
} nimLexer;
...
typedef struct
{
  ...
  /* these fields for captured symbol information */
  vString* symbol;
  vString* scope;
  int lineNumber;
  fpos_t filePosition;
...
static void initLexer(nimLexer* lex)
{
  ...
  lex->operatorList = stringListNew();
  fillLexer(lex);
  skipUTF8BOM(lex);
}
...
{
  stringListDelete(lex->operatorList);
...
static void getSymbol(nimLexer* lex, nimToken* tok)
{
  int pos = lex->bufpos;
  const unsigned char* buf = lex->buf;
...
static void endOperator(nimLexer* lex, nimToken* tok, int pos)
{
  ...
  if(tok->ident == -1)
  {
    vString *newOperator = vStringNew();
    vStringCopy(newOperator, tok->literal);
    stringListAdd(lex->operatorList, newOperator);
    tok->ident = (int) (wInvalid + lastIdent);
    addKeyword(newOperator->buffer, nimLang, tok->ident);
    lastIdent += 1;
  }

  if((tok->ident < oprLow) || (tok->ident > oprHigh))
  {
    tok->tokType = tkOpr;
  }
  else
  {
    tok->tokType = (tokenType) (tok->ident - oprLow + tkColon);
  }
...
static void parseToken(nimLexer* lex, nimToken* tok)
{
  int c = lex->buf[lex->bufpos];
  switch(c)
  {
...
static void rawGetTok(nimLexer* lex, nimToken* tok)
{
  unsigned char c;
  fillToken(tok);
  ...
  if(lex->bufpos >= lex->buflen) return;
...
static void getToken(nimLexer* lex, nimToken* tok)
{
  if(lex->bufpos >= lex->buflen)
  {
    if(!fillLexer(lex))
    {
      tok->indent = 0;
...
static tokenType getTok(nimParser* nim)
{
  nim->prevTokenType = nim->tok.tokType;
...
static void debug_tag(int line, const char* func, nimParser* nim, nimKind kind)
{
  if(debug(DEBUG_PARSE))
  {
    printf("TRACE TAG:%d Callee:%s symbol:%s\n", line, func, nim->symbol->buffer);
  }
  makeNimTag(nim, kind);
}
...
static void debug_nesting(int nestLevel, int line, const char* func, const char* parser)
{
  if(debug(DEBUG_PARSE))
  {
    ...
    printf("Line:%d Caller:%s Callee:%s\n", line, func, parser);
  }
}
...
static void fillSymbol(nimParser* nim)
{
...
static TNodeKind parseSymbol(nimParser* nim, boolean allowNil)
{
  ...
  switch(nim->tok.tokType)
  {
    ...
    case tkAccent:
      ...
      while(TRUE)
      {
        if(nim->tok.tokType == tkAccent)
        {
          if(nim->kind != K_PARAM_LIST) vStringCat(nim->symbol, nim->tok.literal);
          break;
        }
...
static TNodeKind primarySuffix(nimParser* nim, TNodeKind kind, int baseIndent)
{
  ...
  */
  ...
  while(nim->tok.indent < 0
    || (nim->tok.tokType == tkDot && nim->tok.indent >= baseIndent))
  {
...
static TNodeKind parseTuple(nimParser* nim, boolean indentAllowed)
{
  ...
  if(nim->tok.tokType == tkBracketLe)
  {
    ...
    {
      if(nim->kind != K_PARAM_LIST && nim->kind != K_IGNORE)
      {
        nim->kind = K_ELEMENT;
      }
...
static TNodeKind primary(nimParser* nim, TPrimaryMode mode)
{
  ...
  switch(nim->tok.tokType)
  {
    case tkTuple:
      {
        nimKind kind;
        tokenType prevTokenType = nim->prevTokenType;
        if(nim->kind != K_PARAM_LIST && (prevTokenType == tkColon || prevTokenType == tkEquals))
        {
          MAKE_NIM_TAG(nim, K_TUPLE);
          nim->kind = K_TUPLE;
        }
        if(!(prevTokenType == tkColon || prevTokenType == tkEquals))
        {
          kind = nim->kind;
          nim->kind = K_IGNORE;
        }
    ...
    case tkGeneric: case tkConcept:
      if(mode == pmTypeDef)
      {
        nimKind kind = nim->kind;
        tokenType prevTokenType = nim->prevTokenType;
        if(nim->kind != K_PARAM_LIST && (prevTokenType == tkColon || prevTokenType == tkEquals))
        {
          MAKE_NIM_TAG(nim, K_TYPE_CLASS);
          nim->kind = K_TUPLE;
        }
        if(!(prevTokenType == tkColon || prevTokenType == tkEquals))
        {
    ...
    case tkPtr:
      if(nim->kind != K_PARAM_LIST && nim->kind != K_FIELD)
      {
        MAKE_NIM_TAG(nim, K_TYPE);
        nim->kind = K_TYPE;
      }
