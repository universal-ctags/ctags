...
{
	return strncmp (++strp, "ns", 2) == 0 && is_space (strp[2]);
}

static int isFunction (const char *strp)
{
	return strncmp (++strp, "defn", 4) == 0 && is_space (strp[4]);
}

static int isQuote (const char *strp)
{
	return strncmp (++strp, "quote", 5) == 0 && is_space (strp[5]);
}
...
static void functionName (vString * const name, const char *dbp)
{
	...
	for (p = dbp; *p != '\0' && *p != '(' && !is_space ((int) *p) && *p != ')';
		p++)
		vStringPut (name, *p);
}
