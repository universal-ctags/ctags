...
static void saveIgnoreToken(const char * ignoreToken)
{
	...
	if(replacement)
	{
		cppMacroReplacementPartInfo * rep = (cppMacroReplacementPartInfo *)eMalloc(sizeof(cppMacroReplacementPartInfo));
...
static void saveMacro(const char * macro)
{
	...
	char tmp = *c;
	*(char *)c = 0;
	CXX_DEBUG_PRINT("Macro identifier '%s'",identifierBegin);
	*(char *)c = tmp;
#endif
	...
	if(*c == '(')
	{
		...
		while(*c)
		{
			...
			if(*c && (*c != ','))
			{
				...
				char tmp = *c;
				*(char *)c = 0;
				CXX_DEBUG_PRINT("Macro parameter %d '%s'",iParamCount,paramBegin[iParamCount]);
				*(char *)c = tmp;
#endif
				iParamCount++;
...
#define ADD_CONSTANT_REPLACEMENT(start,len) \
		do { \
			...
			{ \
				...
				CXX_DEBUG_PRINT("Constant replacement part changed: '%s'",vStringValue(lastReplacement->constant)); \
			} else { \
		...
		while(*c)
		{
			if(isalpha(*c) || (*c == '_'))
			{
				...
				char tmp = *c;
				*(char *)c = 0;
				CXX_DEBUG_PRINT("Check token '%s'",tokenBegin);
				*(char *)c = tmp;
#endif
				...
				for(;i<iParamCount;i++)
				{
					...
					if(
							(bIsVarArg && (paramLen == 3) && (strncmp(paramBegin[i],"...",3) == 0)) ||
							((!bIsVarArg) && (paramLen == tokenLen) && (strncmp(paramBegin[i],tokenBegin,paramLen) == 0))
						)
					{
						// parameter!
						cppMacroReplacementPartInfo * rep = (cppMacroReplacementPartInfo *)eMalloc(sizeof(cppMacroReplacementPartInfo));
						rep->parameterIndex = i;
						rep->flags = nextParameterReplacementFlags | (bIsVarArg ? CPP_MACRO_REPLACEMENT_FLAG_VARARGS : 0);
						rep->constant = NULL;
						rep->next = NULL;
			...
			if(*c == '#')
			{
				...
				if(*c == '#')
				{
					...
					if(lastReplacement && (lastReplacement->parameterIndex == -1))
					{
						...
						const char * v = vStringValue(lastReplacement->constant);
						const char * p = v + vStringLength(lastReplacement->constant);
						while(p > v)
						{
							p--;
							if(!isspacetab(*p))
						...
						vStringTruncate(lastReplacement->constant,p-v);
						CXX_DEBUG_PRINT("Last replacement truncated to '%s'",vStringValue(lastReplacement->constant));
					}
				} else {
					// stringification
...
static void freeMacroInfo(cppMacroInfo * info)
{
	...
	{
		cppMacroReplacementPartInfo ** p = &(info->replacements);
		while(*p)
		{
			if((*p)->constant)
				vStringDelete((*p)->constant);
			*p = (*p)->next;
		}
	}
	eFree(info);
