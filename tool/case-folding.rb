      ...
      if item
        ...
        when item.lower  then  flags += '|D'
          ...
          unless from=='03B9' or from=='03BC'
            warn 'Unpredicted case 0; check data or adjust program (tool/case_folding.rb).'
          end
        end
        unless item.upper == item.title
          if item.code == item.title
            warn 'Unpredicted case 1; check data or adjust program (tool/case_folding.rb).'
          elsif item.title==to[1]
            flags += '|ST'
          else
            warn 'Unpredicted case 2; check data or adjust program (tool/case_folding.rb).'
          end
        end
      end
    end
