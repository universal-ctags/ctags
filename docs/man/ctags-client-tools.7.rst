...
ctags command generated the tags file: whether the
...
object in a input file. A pseudo-tag is for the tags file
...
regular-tags as described in :ref:`tags(5) <tags(5)>`. An exception is names of
pseudo-tags start with "!_". For the general information about
pseudo-tags, see "TAG FILE INFORMATION" in :ref:`tags(5) <tags(5)>`.

An example of pseudo tags::
...
This pseudo-tag represents "the function kind of C language is enabled
...
``--pseudo-tags=[+|-]names|*``
	...
	If names is preceded by either the '+' or '-' character, the
	effect of each name is added to, or removed from, those currently
	enabled; otherwise the names replace any current settings. All
